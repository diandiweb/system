...
let signheader = $.getdata('dyjssignheader')
let signcookie = $.getdata('dyjssigncookie')

let stepheader = $.getdata('dyjsstepheader')
let stepkey = $.getdata('dyjsstepkey')

let readheader = $.getdata('dyjsreadheader')
let readkey = $.getdata('dyjsreadkey')
...
Object.keys(readheader).forEach((item) => {
    ...
    signheaderArr.push($.getdata('dyjssignheader'))
    signcookieArr.push($.getdata('dyjssigncookie'))
    stepheaderArr.push($.getdata('dyjsstepheader'))
    stepkeyArr.push($.getdata('dyjsstepkey'))
    readheaderArr.push($.getdata('dyjsreadheader'))
    readkeyArr.push($.getdata('dyjsreadkey'))
    let dyjsbcount = ($.getval('dyjsbcount') || '1');
  for (let i = 2; i <= dyjsbcount; i++) {
    signheaderArr.push($.getdata(`dyjssignheader${i}`))
    signcookieArr.push($.getdata(`dyjssigncookie${i}`))
    stepheaderArr.push($.getdata(`dyjsstepheader${i}`))
    stepkeyArr.push($.getdata(`dyjsstepkey${i}`))
    readheaderArr.push($.getdata(`dyjsreadheader${i}`))
    readkeyArr.push($.getdata(`dyjsreadkey${i}`))
  }
}
!(async () => {
if (!signheaderArr[0]) {
...
function GetCookie() {
 if($request&&$request.url.indexOf("aweme" && "sign_in")>=0) {
  const signheader = $request.url.split(`?`)[1]
    if (signheader) $.setdata(signheader,`dyjssignheader${$.idx}`)
    $.log(`[${$.jsname}] 获取sign请求: 成功,signheader: ${signheader}`)
    $.msg(`获取signheader: 成功🎉`, ``)
   const signcookie = $request.headers['Cookie']
  if(signcookie)        $.setdata(signcookie,`dyjssigncookie${$.idx}`)
 ...
 if($request&&$request.url.indexOf("aweme" && "step_submit")>=0) {
	  const stepheader = $request.url.split(`?`)[1]
	    if (stepheader) $.setdata(stepheader,`dyjsstepheader${$.idx}`)
	    $.log(`[${$.jsname}] 获取step请求: 成功,stepheader: ${stepheader}`)
	    $.msg(`获取stepheader: 成功🎉`, ``)
	   const stepkey = JSON.stringify($request.headers)
	  if(stepkey)        $.setdata(stepkey,`dyjsstepkey${$.idx}`)
 ...
 if($request&&$request.url.indexOf("aweme" && "done/read")>=0) {
	  const readheader = $request.url.split(`?`)[1]
	    if (readheader) $.setdata(readheader,`dyjsreadheader${$.idx}`)
	    $.log(`[${$.jsname}] 获取read请求: 成功,readheader: ${readheader}`)
	    $.msg(`获取readheader: 成功🎉`, ``)
	   const readkey = JSON.stringify($request.headers)
	  if(readkey)        $.setdata(readkey,`dyjsreadkey${$.idx}`)
